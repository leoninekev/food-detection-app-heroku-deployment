...
import base64#270619
import cv2

import requests#020719
...
@app.route('/querry-url')#010219 try /querry-url?image=https://train_on_gcloud/flask_experiments/3.jpg
def querry_with_imagurl():
    tick= time.time()
    image_url= request.args.get('image')
    response = requests.get(image_url)
    
    img= Image.open(io.BytesIO(response.content))#250619
...
@app.route('/querry-b64-json', methods=['POST'])#300619 @cmd try curl -X POST -F image=@{'img_data':'ac2a1ca5scab64'}.jpg 'http://localhost:8001/b64-json-querry'
...
if __name__ == '__main__':
    ...
    app.run(host='0.0.0.0', port=os.environ.get("PORT",8001), debug=True)
