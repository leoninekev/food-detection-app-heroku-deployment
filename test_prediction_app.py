...
class b_plate(object):
    def __init__(self,config, model_rpn, model_classifier):# 240519 ,config)        
        self._config = config #300519#this config holds the updated config dict generated after training
        self._model_rpn = model_rpn
        self._model_classifier = model_classifier

    def format_img_size(self, image):
        ...
        return img, ratio#this img is trasformed image
    ...
    def format_img(self, image):#140519
        """ formats an image for model prediction based on config """
        img, ratio = self.format_img_size(image)# 220519#added self to call class function ,self._config)
        img = self.format_img_channels(img)
        return img, ratio


    def preprocess(self, image): # 300519 defined explicitly
    ...
    def get_real_coordinates(self, ratio, x1, y1, x2, y2):#220519; added self as class syntax errpr #140519
    ...
    def postprocess(self, bounding_boxes, probabilities):  # 300519 defined explicitly
        all_dets = []
...
def make_prediction():
    if request.method=='POST':
        ...
        with graph.as_default():
            #creates an object from b_plate class, performing image processing
            overhead = b_plate(config, model_rpn, model_classifier)
            [bounding_boxes, probabilities] = overhead.preprocess(img)#preprocessed outputs
            results = overhead.postprocess(bounding_boxes, probabilities)
